...
// dist is torch::kFloat32 tensor
void AliasSample::Init(const torch::Tensor& dist) {
    auto n = dist.size(0);
    // probability list
    prob = torch::zeros(n, dist.options().dtype(torch::kFloat32));
    // alias list
    alias = torch::zeros(n, dist.options().dtype(torch::kInt64));

    std::stack<int64_t> smaller, larger;
    auto dist_d = dist.to(torch::kFloat32).data<float>();
